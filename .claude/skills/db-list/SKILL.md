...
2. Если пользователь указал базу — ищи совпадение в таком порядке:
   ...
   2. По `aliases` (точное совпадение в массиве)
   3. По `branches` (совпадение с текущей Git-веткой)
   4. По `name` (нечёткое совпадение)
...
> **Важно**: между `/N` и именем пробела нет. Между `/P` и паролем пробела нет. Если пароль пустой — `/P""` или опустить `/P`.
